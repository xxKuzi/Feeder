...
mod motor_test {
    ...
    pub fn run() -> Result<(), String> {
        ...
        let mut pins = Pins::new(pulse, limit_1, limit_2, direction, enable)?;
        pins.log_limit_states();
...
#[cfg(target_os = "linux")]
fn main() {
    if let Err(e) = motor_test::run() {
        println!("Motor test failed: {}", e);
    }
}

#[cfg(not(target_os = "linux"))]
fn main() {
    println!("motor_test is only supported on Linux (Raspberry Pi)");
}
