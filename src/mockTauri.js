...
const isDemoMode = !isTauri;

if (isDemoMode) {
  console.log(
    "%c[Tauri Mock Mode] Initializing mock system for web showcase...",
      ...
      switch (cmd) {
        ...
        case "add_user": {
          const users = JSON.parse(
            localStorage.getItem("feeder_users") || "[]",
          );
          const newUser = {
            user_id: Date.now(),
            name: args.name,
        ...
        case "rename_user": {
          let users = JSON.parse(localStorage.getItem("feeder_users") || "[]");
          users = users.map((u) =>
            u.user_id === args.userId ? { ...u, name: args.newName } : u,
          );
          localStorage.setItem("feeder_users", JSON.stringify(users));
          return null;
        }
        ...
        case "load_current_data": {
          let currentUser = JSON.parse(
            localStorage.getItem("feeder_current_user"),
          );
          const users = JSON.parse(
            localStorage.getItem("feeder_users") || "[]",
          );
            ...
            return [
              {
                ...
                last_calibration:
                  localStorage.getItem("feeder_last_calibration") ||
        ...
        case "add_mode": {
          const modes = JSON.parse(
            localStorage.getItem("feeder_modes") || "[]",
          );
          const newMode = {
            mode_id: Date.now(),
            name: args.data.name,
        ...
        case "add_record": {
          const records = JSON.parse(
            localStorage.getItem("feeder_records") || "[]",
          );
          const newRecord = {
            record_id: Date.now(),
            name: args.data.name,
        ...
        case "add_basket_points": {
          ...
          const current = Number(
            localStorage.getItem("feeder_basket_score") || 0,
          );
